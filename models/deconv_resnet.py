...
class DeconvResNet(nn.Module):
    ...
    def __init__(
        ...
        self.uplayer1 = self._make_layer(
            block, 512, 256, layers[3], stride=2,
            dilate=replace_stride_with_dilation[2])
        self.uplayer2 = self._make_layer(
            block, 256, 128, layers[2], stride=2,
            dilate=replace_stride_with_dilation[1])
        self.uplayer3 = self._make_layer(
            block, 128, 64, layers[1], stride=2,
    ...
    def _make_layer(
        ...
        if stride != 1 or inplanes * block.expansion != planes:
            unsample = nn.Sequential(
                deconv1x1(inplanes * block.expansion, planes, stride),
                norm_layer(planes),
            )
        ...
        for _ in range(1, blocks):
            layers.append(
                block(
                    inplanes * block.expansion, inplanes,
                    stride=1,
                    groups=self.groups,
        ...
        layers.append(
            block(
                inplanes * block.expansion, planes,
                stride=stride,
                sample=unsample,
                groups=self.groups,
                base_width=self.base_width,
